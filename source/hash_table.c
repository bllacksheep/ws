...
static inline uint32_t ht_get_hash(const uint8_t *item_key,
...
int main() {
  ...
  tls_map_insert(&tls_map, host_header, host_header_len, (const uint8_t
*)"example.com"); tls_map_insert(&tls_map, upgrade_header, upgrade_header_len,
(const uint8_t *)"sure"); tls_map_insert(&tls_map, hos_header, hos_header_len,
(const uint8_t *)"fake"); tls_map_insert(&tls_map, connection_header,
connection_header_len, (const uint8_t *)"keep-alive");

  const uint8_t *v1 = tls_map_lookup(&tls_map, host_header, host_header_len);
  printf("Host is: %s\n", v1);
  const uint8_t *v2 = tls_map_lookup(&tls_map, upgrade_header,
upgrade_header_len); printf("Upgrade is: %s\n", v2); const uint8_t *v3 =
tls_map_lookup(&tls_map, hos_header, hos_header_len); printf("Hos is: %s\n",
v3); const uint8_t *v4 = tls_map_lookup(&tls_map, connection_header,
connection_header_len); printf("Connection is: %s\n", v4);

  // new request
  tls_map.epoch++;
  tls_map_insert(&tls_map, host_header, host_header_len, (const uint8_t
*)"new.com"); tls_map_insert(&tls_map, upgrade_header, upgrade_header_len,
(const uint8_t *)"no"); tls_map_insert(&tls_map, hos_header, hos_header_len,
(const uint8_t *)"real"); tls_map_insert(&tls_map, connection_header,
connection_header_len, (const uint8_t *)"close");

  const uint8_t *v5 = tls_map_lookup(&tls_map, host_header, host_header_len);
  printf("Host is: %s\n", v5);
  const uint8_t *v6 = tls_map_lookup(&tls_map, upgrade_header,
upgrade_header_len); printf("Upgrade is: %s\n", v6); const uint8_t *v7 =
tls_map_lookup(&tls_map, hos_header, hos_header_len); printf("Hos is: %s\n",
v7); const uint8_t *v8 = tls_map_lookup(&tls_map, connection_header,
connection_header_len); printf("Connection is: %s\n", v8);
}
*/
