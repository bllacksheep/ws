...
unsigned int check_path(const char *uri) {
...
// raw_req_t is const
raw_req_t parse_raw_bytes(const char *req, int len) {
  unsigned int valid = 0;
  unsigned int invalid = -1;

  if (len < 4)
    return (raw_req_t){.is_http = invalid};
...
// rfc 9110
req_t parser_basic(const char *raw_buf, int buf_len) {
  raw_req_t raw_req = parse_raw_bytes(raw_buf, buf_len);
  if (raw_req.is_http == -1)
    return (req_t){.request_method = UNKNOWN};
...
const char *http_parse_request(const char *stream) {
  _parser_parse_request(stream);
}
...
const char *http_handle_raw_request_stream(const char *raw_req_buf,
  ...
  http_parse_request(raw_req_buf);
